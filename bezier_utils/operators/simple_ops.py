...
class JoinBezierSegsOp(Operator):
    ...
    def execute(self, context):
        ...
        newCurve = joinSegs(
            curves, optimized=optimized, straight=straight, margin=mergeDist
        )
...
class RemoveDupliVertCurveOp(Operator):
    ...
    bl_options = {"REGISTER", "UNDO"}
    bl_description = (
        "Remove duplicate vertices and mark splines as cyclic if applicable"
    )
    ...
    def execute(self, context):
        ...
        for curve in curves:
            removeDupliVert(
                curve, bpy.context.window_manager.bezierToolkitParams.dupliVertMargin
            )
    ...
    def execute(self, context):
        ...
        for curve in curves:
            ...
            if fillType in advanced_fill_types:
                ...
                meshObj, center, normal = convertToFace(
                    curve, remeshRes, perSeg, fillType, optimized
                )
...
class AlignToFaceOp(Operator):
    ...
    bl_description = (
        "Align all points of selected curves with " + "nearest face of selected meshes"
    )
...
class PasteLengthOp(Operator):
    ...
    def execute(self, context):
        ...
        if src is not None and isBezier(src):
            dests = [
                o for o in bpy.context.selected_objects if (isBezier(o) and o != src)
            ]
            if len(dests) > 0:
    ...
    def execute(self, context):
        ...
        if len(curves) < minCurves:
            ...
            intersectCurves(
                curves,
                params.intersectOp,
                params.intersectNonactive,
                params.intersectMargin,
                rounding,
                params.selfIntersect,
            )
...
class BooleanCurvesOp(Operator):
    ...
    def execute(self, context):
        ...
        otherCurves = [
            o for o in bpy.context.selected_objects if isBezier(o) and o != actCurve
        ]
        ...
        result = booleanCurves(
            curves, params.booleanOp, params.intersectMargin, rounding
        )
        ...
        if result:
            ...
            self.report(
                {"INFO"},
                f"Boolean {params.booleanOp} on {len(curves)} curves completed",
            )
        else:
...
class ExportSVGOp(Operator):
    ...
    # User input
    clipView: BoolProperty(
        name="Clip View", description="Clip objects to view boundary", default=True
    )

    exportView: EnumProperty(
        name="Export View", items=getExportViewList, description="View to export"
    )

    lineWidth: FloatProperty(
        name="Line Width", description="Line width in exported SVG", default=3, min=0
    )

    lineColorOpts: EnumProperty(
        name="Line Color",
        items=(
            ("RANDOM", "Random", "Use random color for curves"),
    ...
    fillColorOpts: EnumProperty(
        name="Fill Color",
        items=(
            ("RANDOM", "Random", "Use random fill color"),
            ("PICK", "Pick", "Pick color"),
        ),
...
class Smart2DProjectOp(Operator):
    """Smart Project 3D Curves to Best-Fit 2D Planes"""
    bl_idname = "object.smart_2d_project"
    bl_label = "Smart 2D Project"
    bl_options = {'REGISTER', 'UNDO'}
    bl_description = "Project selected 3D curves onto their best-fit 2D planes"
    ...
    def execute(self, context):
        ...
        if not curves:
            self.report({'WARNING'}, "No Bezier curves selected")
            return {'CANCELLED'}
        ...
        self.report({'INFO'}, f"Projected {projected_count} curve(s) to best-fit 2D plane")
        return {'FINISHED'}
