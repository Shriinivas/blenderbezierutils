...
from ..utils.bezier_math import get3DVector, getSegsForArc, getWSDataForSegs, getInterpBezierPts, getInterpolatedVertsCo
...
class Primitive2DDraw(BaseDraw):
    ...
    def getCurvePts(self, numSegs, axisIdxs, z = None):
        ...
        if(curvePts is None): return None
        
        curvePts = [[tm.inverted_safe() @ p if not isinstance(p, str) \
...
class EllipseDraw(ClosedShapeDraw):
    ...
    def getShapePts(self, mode, numSegs, bbStart, bbEnd, center2d, startAngle, \
        ...
        newSegs = []
        for i, co in enumerate(vertCos):
            newSegs.append([co, co, co, 'VECTOR', 'VECTOR'])
